...
interface DitheredWavesProps {
  ...
  waveBaseColor: [number, number, number]
  colorNum: number
  pixelSize: number
...
function DitheredWaves({
  ...
  const waveUniformsRef = useRef<WaveUniforms>({
    ...
    baseColor: new THREE.Uniform(new THREE.Color(...waveBaseColor)),
  ...
  useEffect(() => {
    if (!enableMouseInteraction || !externalMouse) return
    const rect = gl.domElement.getBoundingClientRect()
    const dpr = gl.getPixelRatio()
    mouseRef.current.set(
      (externalMouse.x - rect.left) * dpr,
      (externalMouse.y - rect.top) * dpr,
    )
  }, [externalMouse, enableMouseInteraction, gl])

  const prevColor = useRef([...waveColor])
  const prevBaseColor = useRef([...waveBaseColor])
  ...
  useFrame(({ clock }) => {
    ...
    if (!prevBaseColor.current.every((v, i) => v === waveBaseColor[i])) {
      ;(u.baseColor.value as THREE.Color).set(...waveBaseColor)
      prevBaseColor.current = [...waveBaseColor]
    }
...
export interface DitherProps {
  ...
  waveAmplitude?: number
  waveColor?: [number, number, number]
  /** Page background color in RGB 0–1. Defaults to black (React Bits demo). */
  waveBaseColor?: [number, number, number]
  colorNum?: number
  pixelSize?: number
  disableAnimation?: boolean
  enableMouseInteraction?: boolean
  mouseRadius?: number
  /** Window mouse position when parent uses pointer-events-none */
  externalMouse?: { x: number; y: number } | null
  className?: string
}
